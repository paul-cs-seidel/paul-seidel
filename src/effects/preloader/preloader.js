/*
 * preloader — Lade-Overlay (Scramble-Loop + drainende Wellen-SVG).
 *
 * Eine GSAP-Timeline scrambelt im Loop die Lade-Nachrichten; nach
 ...
 *
 * Der Preloader läuft auf Timer; `finish()` erlaubt einen vorzeitigen
 * Abschluss. Antrieb: GSAP + ScrambleTextPlugin. Werte: ./preloader.config.js.
...
import {
  DEFAULT_PRELOADER_OPTIONS,
  PRELOADER_CLASSES,
  PRELOADER_PAGE,
} from './preloader.config.js';
...
const EASES = Object.fromEntries(
  Object.entries(DEFAULT_PRELOADER_OPTIONS.eases).map(([key, { name, path }]) => [
    key,
...
export function mount(root = globalThis.document?.body, options = {}) {
  ...
  const drain = () => {
    ...
    tl.to(waveState, {
      ...cfg.wave.mid,
      duration: cfg.drain.first.duration,
      ease: EASES[cfg.drain.first.ease],
      onUpdate,
    }).to(
      waveState,
      {
        ...cfg.wave.empty,
        duration: cfg.drain.second.duration,
        ease: EASES[cfg.drain.second.ease],
        onUpdate,
      },
      cfg.drain.second.overlap,
    );
    timelines.push(tl);
  };
  ...
  const finish = () => {
    ...
    tl.set(text, { autoAlpha: 1, opacity: 1, y: 0 })
      .to(text, {
        ...
        ease: f.in.ease,
        scrambleText: {
          chars: cfg.scrambleChars,
          revealDelay: f.in.revealDelay,
          speed: f.in.speed,
          text: cfg.farewell,
        },
      })
      .to({}, { duration: f.hold })
      .to(text, {
        duration: f.out.duration,
        ease: f.out.ease,
        opacity: f.out.opacity,
        scrambleText: {
          chars: cfg.scrambleChars,
          revealDelay: f.out.revealDelay,
          speed: f.out.speed,
          text: '',
        },
      })
  ...
  const startLoop = () => {
    ...
    cfg.messages.forEach((message) => {
      loop
        .to(text, {
          duration: l.in.duration,
          ease: l.in.ease,
          opacity: l.in.opacity,
          y: l.in.y,
          scrambleText: {
            chars: cfg.scrambleChars,
            revealDelay: l.in.revealDelay,
            speed: l.in.speed,
            text: message,
          },
        })
        .to({}, { duration: l.hold })
        .to(text, {
          duration: l.out.duration,
          ease: l.out.ease,
          opacity: l.out.opacity,
          scrambleText: {
            chars: cfg.scrambleChars,
            revealDelay: l.out.revealDelay,
            speed: l.out.speed,
            text: '',
          },
        })
